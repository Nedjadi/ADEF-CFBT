...
class db_tuple {
	 ...
	 * @retour	vide
	 */
    protected function __construct($label_id, $id) {
		$this->id = $id;
		$nom_classe = get_class($this);
		$this->nom_table = str_replace("db_", "".DB_PREFIXE_TABLE, $nom_classe);
	...
	public function supprimer() {
		db_connexion::Supprimer_tuple($this->label_id, $this->id, $this->nom_table);
	}
	...
	public function en_db() {
		$en_db = db_connexion::Chercher_tuple($this->label_id, $this->id, $this->nom_table);
		return $en_db;
	}
	 ...
	 * @retour	valeur
	 */
	public function lire($propriete) {
	 ...
	 * @retour	vide
	 */
	public function ecrire($propriete, $valeur) {
		if (!(isset($this->proprietes_types[$propriete]))) {
